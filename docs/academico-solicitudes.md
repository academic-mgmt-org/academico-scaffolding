# Interfaz independiente: academico-solicitudes

Esta receta instala únicamente Solicitudes académicas: creación, seguimiento,
asignación, cambios de estado, documentos, resolución y cancelación. Auth se
compila solo para la sesión; no se instalan Usuarios, Matrículas,
Calificaciones ni Notificaciones. Los datos del estudiante se ingresan en la
propia interfaz.

## Crear e instalar

```bash
# ===== INICIO: INTERFAZ SOLO PARA ACADEMICO-SOLICITUDES =====
set -Eeuo pipefail

SCAFFOLDING_DIR='/home/azureuser/academico-scaffolding'
WORK_ROOT="${WORK_ROOT:-$HOME/interfaces-academicas}"
APP_NAME="${APP_NAME:-interfaz-academico-solicitudes}"
...
"$SCAFFOLDING_DIR/setup/prepare-interface-runtime.sh" \
  "$WORK_ROOT/$APP_NAME"
# ===== FIN: INTERFAZ SOLO PARA ACADEMICO-SOLICITUDES =====
```
...
El resultado compila exactamente `auth_v1.proto` y `solicitudes_v1.proto`. La
pantalla de acceso está en `/academico/login` y el flujo de
`SOLICITUDES_BECA_REVISION_RESOLUCION.md` queda en `/academico/solicitudes`.

## Ejecutar y comprobar

```bash
cd "${WORK_ROOT:-$HOME/interfaces-academicas}/${APP_NAME:-interfaz-academico-solicitudes}"
